 ...
 */
public class BreakerTalonFX extends TalonFX implements BreakerSmartMotorControler {

	private String _description, deviceName, faultStr;
    ...
    public BreakerSmartMotorControlRequestStatusCode setControl(BreakerSmartMotorControlRequest request) {
        ...
        BreakerSmartMotorControlRequestStatusCode sc = BreakerSmartMotorControlRequestStatusCode.OK;
        if ((request.getEnableFOC() || pidSlot > 2) || request.getEnableRequestFallback()) {
            if (request.getEnableFOC()) {
                enableFOC = false;
            }
            if (pidSlot > 2) {
                pidSlot = 0;
            }
            sc = BreakerSmartMotorControlRequestStatusCode.FALLBACK;
        } else {
            return BreakerSmartMotorControlRequestStatusCode.FAIL;
        }
    ...
    public void runSelfTest() {
        faultStr = "";
        Pair<DeviceHealth, String> pair = BreakerCTREUtil.checkMotorFaultsAndConnection(this);
